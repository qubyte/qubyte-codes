...
content in the file name, and serves it with immutable[^][[Chrome doesn't
support immutable][chrome immutable], but immutable is paired with a long
`max-age` and the result in Chrome is very similar.] cache headers, like the CSS
mentioned above.
