...
async function waitForDependencies(executionGraph, dependencies) {
  ...
  await Promise.all(dependencies.map(async name => {
    const dependecyNode = executionGraph.nodes.get(name);
    ...
    const [result] = await once(executionGraph, `done:${name}`);
...
module.exports = class ExecutionGraph extends EventEmitter {
  constructor(parent) {
    super();
  ...
  async addNode({ name, action, dependencies = [] }) {
    ...
    this.emit(`done:${name}`, node.result);
    this.emit('done', name, node.result);
