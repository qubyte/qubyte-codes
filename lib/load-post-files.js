...
import { createSocialImage } from './create-social-image.js';
...
export class PostPage extends Page {
  ...
  constructor({
    ...
    this.draft = draft;
    this.slug = slug;
    this.mastodonHandle = '@aura@sunny.garden';
    this.title = title;
...
async function loadPostFile({ fileUrl, basePath, baseUrl, repoUrl, type, extraCss, hashedScripts, images }) {
  ...
  const socialImageSeed = createHash('md5')
    .update(slug)
    .digest()
    .readUInt32BE();
  await createSocialImage(socialImageSeed);
  const localUrl = type ? `/${type}/${slug}` : `/${slug}`;
  const { content, footnotes } = render(body, images);
  ...
  const digested = new PostPage({
    ...
    updatedAt,
    editUrl,
    ogImage: `/images/social-${socialImageSeed}.png`
  });
