...
export default async function loadPostFiles(dir, baseUrl, type, extraCss) {
  const filenames = await fs.readdir(dir);
  const posts = await Promise.all(filenames.map(fn => loadPostFile(path.join(dir, fn), baseUrl, type, extraCss)));
  const now = Date.now();
