...
function buildRuby(elements) {
  ...
  for (let i = 0; i < elements.length; i += 2) {
    const base = handlebars.Utils.escapeExpression(elements[i]);
    const text = elements[i + 1];

    if (!text) {
      content.push(`${base}<rt></rt>`);
    } else {
      content.push(`${base}<rp>(</rp><rt>${handlebars.Utils.escapeExpression(text)}</rt><rp>)</rp>`);
    }
  }
