...
function postrender(string) {
  ...
  // Group all elements by language.
  const elementsByLang = {};

  for (const lang of langs) {
    elementsByLang[lang] = Array.from(dom.window.document.querySelectorAll(`:lang(${lang})`));
  }
  ...
  for (const el of elementsWithLang) {
    if (elementsByLang[el.lang].includes(el.parentElement)) {
      changes = true;
