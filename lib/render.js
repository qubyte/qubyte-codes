...
function addFootnotes(window, footnotes) {
  if (!footnotes.length) return false;
    ...
    extensions: [
      {
        ...
        renderer(token) {
          ...
          if (!addLang) return this.parser.parseInline(token.inner);
      ...
      },
      {
        ...
        renderer(token) {
          ...
          for (const el of math.querySelectorAll('[style]')) el.removeAttribute('style');
