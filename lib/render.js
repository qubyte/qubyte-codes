...
// Remove the lang attribute of an element when its parent has the same lang
// attribute. This isn't very sophisticated, but it does address the most common
// issue of ruby elements nested in spans with the same lang attribute. This
// function does its best to do the least it can.
function postrender(string) {
  const dom = new JSDOM(string);
  const elementsWithLang = dom.window.document.querySelectorAll('[lang]');

  if (!elementsWithLang.length) {
    return string;
  }
  ...
  for (const el of elementsWithLang) {
    if (el.parentElement && el.parentElement.lang === el.lang) {
      changes = true;
...
module.exports = function render(string) {
  ...
  return marked(prerender(string), { highlight, renderer }).then(postrender);
};
