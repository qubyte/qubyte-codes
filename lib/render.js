...
const texToSvg = mathjax.document('', { InputJax: tex, OutputJax: svg });
...
marked.use({
  ...
  renderer: {
    code(code, lang) {
      // Mathematics borrows the code type. Pass it through since it's already
      // rendered.
      return lang === 'mathematics' ? code : false;
    }
  }
});
  ...
  const rendered = marked(string, {
    walkTokens(token) {
      if ((token.raw.startsWith('\\[\n') && token.raw.endsWith('\n\\]')) || (token.raw.startsWith('$$\n') && token.raw.endsWith('\n$$'))) {
        ...
        token.text = renderMaths(token.raw.slice(3, -3), `equation-${equationId}`);
      }
    }
  });
