  ...
  }),
  {
    ...
    hooks: {
      ...
      postprocess(html) {
        ...
        return spansRemoved ? window.document.body.innerHTML : html;
      }
    }
  }
);
 ...
 * @returns {{content: string, footnotes: string[]}}
 */
export default function render(input, imagesMap = new Map()) {
  const footnotes = [];
  const content = marked(input, { async: false, imagesMap, footnotes });
