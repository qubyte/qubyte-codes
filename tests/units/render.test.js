...
test('render', () => {
  ...
  test('returns rendered content', () => {
    const rendered = render('a paragraph', URL);

    assert.equal(rendered.trim(), '<p>a paragraph</p>');
  });

  test('renders $$ delimited blocks labelled as SVG in the img role and a title containing the original maths', () => {
  ...
  test('mathematics with dollar delimiters', () => {
    test('removes inline style from rendered mathematics', () => {
      const rendered = render('$$\na=b\n$$', URL);
    ...
    test('updates the ID of the title of the rendered mathematics', () => {
      const rendered = render('$$\na=b\n$$', URL);
    ...
    test('increments the title ID to avoid collisions', () => {
      const rendered = render('$$\na=b\n$$\n\ntext\n\n$$\nc=d\n$$', URL);
      const { window: { document } } = new JSDOM(rendered);
      const titles = document.querySelectorAll('svg title[id]');
    ...
    test('sets aria-labelledby to the title element ID for each snipped', () => {
      const rendered = render('$$\na=b\n$$\n\ntext\n\n$$\nc=d\n$$', URL);
  ...
  test('inline ruby links', () => {
    test('renders ruby elements', () => {
  ...
  test('highlighted text', () => {
    test('renders text surrounded by pairs of == in mark tags', () => {
    ...
    test('renders highlighted text inside another inline element', () => {
      const rendered = render('_==text==_').trim();

      assert.equal(rendered, '<p><em><mark>text</mark></em></p>');
    });

    test('renders highlighted text with another inline element within', () => {
  ...
  test('lang attributes', () => {
    test('retains spans with a lang attribute when they have no child elements (only text nodes)', () => {
      const rendered = render('{ab:xyz} def').trim();

      assert.equal(rendered, '<p><span lang="ab">xyz</span> def</p>');
    });

    test('removes the span when the span has an element as its single child node and copies over the lang attribute', () => {
      const rendered = render('{ab:*xyz*}').trim();

      assert.equal(rendered, '<p><em lang="ab">xyz</em></p>');
    });

    test('retains spans with lang attributes when they have more than one node', () => {
      const rendered = render('{ab:*xx* yy} zz').trim();

      assert.equal(rendered, '<p><span lang="ab"><em>xx</em> yy</span> zz</p>');
    });

    test('moves the lang and children of the span to its parent element when the parent element has the span as its only child', () => {
  ...
  test('footnotes', () => {
    test('does not add a footnotes section when there are no footnotes', () => {
    ...
    test('places the text of a footnote in a footnotes section', () => {
    ...
    test('renders inline markdown in footnote text', () => {
    ...
    test('links the location of the footnote to the footnote text', () => {
      const rendered = render('abc[^][a footnote] def').trim();
    ...
    test('links the footnote text to the location of the footnote', () => {
      const rendered = render('abc[^][a footnote] def').trim();
      const { window: { document } } = new JSDOM(rendered, { url: 'http://example.test/' });
    ...
    test('renders the footnote ref as superscript with a number in square brackets', () => {
      const rendered = render('abc[^][a footnote] def').trim();
      const { window: { document } } = new JSDOM(rendered);
    ...
    test('renders the footnote ref with a reverse link at the end', () => {
      const rendered = render('abc[^][a footnote] def').trim();
      const { window: { document } } = new JSDOM(rendered);
    ...
    test('labels each ref sequentially', () => {
